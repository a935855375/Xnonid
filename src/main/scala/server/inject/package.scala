...
/**
  ...
  *
  * Components are bound in the DI container.  Each binding is identified by a [[server.inject.BindingKey BindingKey]], which is
  * typically an interface that the component implements, and may be optionally qualified by a JSR-330 qualifier
  * annotation. A binding key is bound to a [[server.inject.BindingTarget BindingTarget]], which describes how the implementation
  ...
  *
  * Bindings are provided by instances of [[server.inject.Module Module]].
  *
  * Out of the box, Play provides an implementation of this abstraction using Guice.
  *
  * @see The [[server.inject.Module Module]] class for information on how to provide bindings.
  */
