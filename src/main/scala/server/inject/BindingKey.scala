...
final case class BindingKey[T](clazz: Class[T], qualifier: Option[QualifierAnnotation]) {
  ...
  def qualifiedWith(name: String): BindingKey[T] =
    qualifiedWith(new server.inject.NamedImpl(name))
  ...
  def to(provider: Provider[_ <: T]): Binding[T] =
    Binding(this, Some(ProviderTarget(provider)), None, eager = false, SourceLocator.source)
  ...
  def to(key: BindingKey[_ <: T]): Binding[T] =
    Binding(this, Some(BindingKeyTarget(key)), None, eager = false, SourceLocator.source)
  ...
  /**
    ...
    */
  def toSelf: Binding[T] = Binding(this, None, None, eager = false, SourceLocator.source)

  override def toString = {
    s"$clazz${qualifier.fold("")(" qualified with " + _)}"
  }

  private def validateTargetNonAbstract[TT](target: Class[TT]): Class[TT] = {
    if (target.isInterface || Modifier.isAbstract(target.getModifiers)) {
      throw new PlayException(
        "Cannot bind abstract target",
        s"""You have attempted to bind $target as a construction target for $this, however, $target is abstract. If you wish to bind this as an alias, bind it to a ${
          classOf[
            BindingKey[_]
            ]
        } instead."""
...
/**
  ...
  *
  * @see The [[server.inject.Module]] class for information on how to provide bindings.
  */
