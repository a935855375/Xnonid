...
import server.libs.reflect.ConstructorUtils
...
abstract class Module {
  ...
  final def bind[T](clazz: Class[T]): BindingKey[T] = server.inject.bind(clazz)

  /**
    * Create a binding key for the given class.
    */
  final def bind[T: ClassTag]: BindingKey[T] = server.inject.bind[T]
