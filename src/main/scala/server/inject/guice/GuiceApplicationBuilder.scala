...
import server._
import server.inject.RoutesProvider
import server.inject.bind
import server.mvc.Handler
import server.mvc.RequestHeader
import server.routing.Router
import server.core.DefaultWebCommands
import server.core.WebCommands
...
final case class GuiceApplicationBuilder(
  ...
  def globalApp(enabled: Boolean): GuiceApplicationBuilder =
    configure(Server.GlobalAppConfigKey -> enabled)
