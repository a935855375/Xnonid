...
import server.core.{BuildLink, SourceMapper, WebCommands}
import server.inject.ApplicationLifecycle
import server.mvc.{ControllerComponents, DefaultControllerComponents}
...
object ApplicationLoader {
  ...
  import server.inject.DefaultApplicationLifecycle
  ...
  private[server] final class NoApplicationLoader extends ApplicationLoader {
  ...
  def apply(context: Context): ApplicationLoader = {
    ...
    Reflect.configuredClass[ApplicationLoader, server.ApplicationLoader, NoApplicationLoader](
      context.environment,
      ...
      case Some(Right(_)) =>
        loaderNotFound()
      /*val javaApplicationLoader: server.ApplicationLoader = javaClass.getDeclaredConstructor().newInstance()
      ...
      class JavaApplicationLoaderAdapter extends ApplicationLoader {
        override def load(context: ApplicationLoader.Context): Application = {
          val javaContext     = new server.ApplicationLoader.Context(context)
