...
import server.data.format._
import server.data.validation._
import server.http.HttpVerbs
import server.templates.PlayMagic.translate
...
case class Form[T](mapping: Mapping[T], data: Map[String, String], errors: Seq[FormError], value: Option[T]) {
  ...
  /**
    ...
    * @return a copy of this form filled with the new data
    */
  def bindFromRequest()(implicit request: server.mvc.Request[_]): Form[T] = {
    bindFromRequest((request.body match {
      case body: server.mvc.AnyContent if body.asFormUrlEncoded.isDefined => body.asFormUrlEncoded.get
      case body: server.mvc.AnyContent if body.asMultipartFormData.isDefined =>
        body.asMultipartFormData.get.asFormUrlEncoded
      case body: server.mvc.AnyContent if body.asJson.isDefined =>
        FormUtils.fromJson(js = body.asJson.get).mapValues(Seq(_))
      case body: Map[_, _] => body.asInstanceOf[Map[String, Seq[String]]]
      case body: server.mvc.MultipartFormData[_] => body.asFormUrlEncoded
      case body: Either[_, server.mvc.MultipartFormData[_]] =>
        body match {
  ...
  /**
    ...
    */
  def errorsAsJson(implicit provider: server.i18n.MessagesProvider): play.api.libs.json.JsValue = {
...
case class FormError(key: String, messages: Seq[String], args: Seq[Any] = Nil) {
  ...
  /**
    ...
    */
  def format(implicit messages: server.i18n.Messages): String = {
...
case class RepeatedMapping[T](
  ...
  def bind(data: Map[String, String]): Either[Seq[FormError], List[T]] = {
    val allErrorsOrItems: Seq[Either[Seq[FormError], T]] =
      RepeatedMapping.indexes(key, data).map(i => wrapped.withPrefix(key + "[" + i + "]").bind(data))
  ...
  def unbindAndValidate(value: List[T]): (Map[String, String], Seq[FormError]) = {
    val (datas, errors) = value.zipWithIndex.map {
      case (t, i) => wrapped.withPrefix(key + "[" + i + "]").unbindAndValidate(t)
    }.unzip
