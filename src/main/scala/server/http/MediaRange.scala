...
import server.http.MediaRange.MediaRangeParser
...
object MediaRange {
  ...
  def preferred(acceptableRanges: Seq[MediaRange], availableMediaTypes: Seq[String]): Option[String] = {
    val acceptableTypes = for {
      mediaRange <- acceptableRanges.sorted(ordering).toStream
  ...
  /**
    ...
    */
  implicit val ordering: Ordering[MediaRange] = new Ordering[server.http.MediaRange] {
    ...
    def compare(a: server.http.MediaRange, b: server.http.MediaRange) = {
  ...
  private[http] object MediaRangeParser extends Parsers {
    ...
    def badPart(p: Char => Boolean, msg: => String) = rep1(acceptIf(p)(ignoreErrors)) ^^ { chars =>
