...
import server.core.{DefaultWebCommands, WebCommands}
import server.http.Status._
import server.inject.Binding
import server.libs.streams.Accumulator
import server.mvc._
import server.routing.Router
import server.{Configuration, Environment, OptionalDevContext}
...
object HttpRequestHandler {
  // todo There we don't set default HttpRequestHandler from config
  def bindingsFromConfiguration(environment: Environment, configuration: Configuration): Seq[Binding[_]] = {
    Seq(server.inject.bind[HttpRequestHandler].to(classOf[DefaultHttpRequestHandler]))
  }
  ...
  class DefaultHttpRequestHandler(
    ...
    override def handlerForRequest(request: RequestHeader): (RequestHeader, Handler) = {
      ...
      webCommandResult match {
        case Some(r) =>
          ...
          (request, ActionBuilder.ignoringBody {
            r
          })
        case None =>
    ...
    /**
      ...
      * default behavior is to wrap all [[server.mvc.EssentialAction]]s by calling `filterAction`, but to leave
