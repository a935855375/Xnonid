...
import server._
import server.core.SourceMapper
import server.core.exception.{PlayException, UnexpectedException, UsefulException}
import server.http.Status._
import server.inject.Binding
import server.libs.exception.ExceptionUtils
import server.mvc.Results._
import server.mvc._
...
trait HttpErrorHandler {
  ...
  /**
    ...
    * @param statusCode The error status code.  Must be greaterF or equal to 400, and less than 500.
...
object HttpErrorHandler {
  ...
  // Todo I changed error handler over default method which set ErrorHandler by reading from config file
  def bindingsFromConfiguration(environment: Environment, configuration: Configuration): Seq[Binding[_]] = {
    Seq(server.inject.bind[HttpErrorHandler].toInstance(DefaultHttpErrorHandler))
  }
}
...
class DefaultHttpErrorHandler(
  ...
  protected def onOtherClientError(request: RequestHeader, statusCode: Int, message: String): Future[Result] = {
    Future.successful(BadRequest("400 Bad Request"))
  }
...
class JsonHttpErrorHandler(environment: Environment, sourceMapper: Option[SourceMapper] = None)
  ...
  /**
    ...
    */
  override def onClientError(request: RequestHeader, statusCode: Int, message: String): Future[Result] = {
    if (server.http.Status.isClientError(statusCode)) {
