...
public final class Paths {
    ...
    private static String[] toSegments(String url) {
        return Arrays.stream(url.split(SEPARATOR)).filter(s -> !s.isEmpty()).toArray(String[]::new);
    }

    private static String[] toParentDirs(int count) {
        return IntStream.range(0, count).mapToObj(i -> PARENT_DIR).toArray(String[]::new);
    }
