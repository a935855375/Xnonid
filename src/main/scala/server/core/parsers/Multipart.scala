...
import server.libs.Files.TemporaryFile
import server.libs.Files.TemporaryFileCreator
import server.libs.streams.Accumulator
import server.mvc._
import server.mvc.MultipartFormData._
import server.http.Status._
import server.http.HttpErrorHandler
...
object Multipart {
  ...
  /**
    * Parses the stream into a stream of [[server.mvc.MultipartFormData.Part]] to be handled by `partHandler`.
    *
  ...
  private[server] object FileInfoMatcher {
  ...
  private[server] object PartInfoMatcher {
