...
object EssentialAction {
  ...
  def apply(f: RequestHeader => Accumulator[ByteString, Result]): EssentialAction =
    (rh: RequestHeader) => f(rh)
}
      ...
      def apply(request: RequestHeader) =
        self(request).mapFuture {
          case Right(a) =>
            ...
            f(a).map(Right.apply)(server.core.Execution.trampoline)
          case left =>
...
object ActionBuilder {
  ...
  /**
    ...
    * consequences, this method is private[server].
    */
  private[server] lazy val ignoringBody: ActionBuilder[Request, AnyContent] =
    new IgnoringBody()(server.core.Execution.trampoline)
}
...
object Action extends DefaultActionBuilder {
  override def executionContext: ExecutionContext = server.core.Execution.internalContext
...
trait ActionFilter[R[_]] extends ActionRefiner[R, R] {
  ...
  protected final def refine[A](request: R[A]) =
