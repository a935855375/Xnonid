...
import server.core.parsers.{FormUrlEncodedParser, Multipart}
import server.data.Form
import server.http.Status._
import server.http.{DefaultHttpErrorHandler, HttpConfiguration, HttpErrorHandler, ParserConfiguration}
import server.libs.Files.{SingletonTemporaryFileCreator, TemporaryFile, TemporaryFileCreator}
import server.libs.streams.Accumulator
import server.mvc.MultipartFormData.{BadPart, FilePart}
import server.utils.PlayIO
import server.{Application, Logger}
...
case class RawBuffer(
  ...
  private[server] def push(chunk: ByteString): Unit = {
    if (inMemory != null) {
  ...
  private[server] def backToTemporaryFile(): Unit = {
...
trait BodyParsers {
  ...
  @inline private def maybeApp = server.Server.privateMaybeApplication.toOption
...
trait BodyParserUtils {
  ...
  def error[A](result: Future[Result]): BodyParser[A] = BodyParser("error") { request =>
    import server.core.Execution.Implicits.trampoline
...
trait PlayBodyParsers extends BodyParserUtils {
  ...
  private[server] implicit def materializer: Materializer

  private[server] def config: ParserConfiguration

  private[server] def errorHandler: HttpErrorHandler

  private[server] def temporaryFileCreator: TemporaryFileCreator
  ...
  private[server] val ApplicationXmlMatcher = """application/.*\+xml.*""".r
  ...
  def tolerantXml(maxLength: Long): BodyParser[NodeSeq] =
    tolerantBodyParser[NodeSeq]("xml", maxLength, "Invalid XML") { (request, bytes) =>
      ...
      server.Server.XML.load(inputSource)
    }
  ...
  def temporaryFile: BodyParser[TemporaryFile] = BodyParser("temporaryFile") { request =>
    ...
    file(tempFile)(request).map(_ => Right(tempFile))(server.core.Execution.Implicits.trampoline)
  }
  ...
  /**
    ...
    */
  private[server] def enforceMaxLength[A](
...
object BodyParsers extends BodyParsers {
  ...
  private[server] def takeUpTo(maxLength: Long): Graph[FlowShape[ByteString, ByteString], Future[MaxSizeStatus]] =
    new TakeUpTo(maxLength)

  private[server] class TakeUpTo(maxLength: Long)
  ...
  private[server] class MaxLengthLimitAttained extends RuntimeException(null, null, false, false)
