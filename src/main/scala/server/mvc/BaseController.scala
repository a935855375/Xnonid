...
import server.i18n.{Lang, Langs, MessagesApi}
...
trait Controller extends ControllerHelpers with BodyParsers {
  ...
  /**
    ...
    * }}}
    *
    * @deprecated This class relies on MessagesApi. Use [[server.i18n.I18nSupport]]
  ...
  implicit def request2lang(implicit request: RequestHeader): Lang = {
    server.Server.privateMaybeApplication
      .map(app => server.i18n.Messages.messagesApiCache(app).preferred(request).lang)
      .getOrElse(request.acceptLanguages.headOption.getOrElse(server.i18n.Lang.defaultLang))
  }
}
