...
import server.MarkerContexts.SecurityMarkerContext
import server.core.encoding.DefaultCookie
import server.http.{CookiesConfiguration, HttpConfiguration, JWTConfiguration, SecretConfiguration}
import server.inject.SimpleModule
import server.libs.Scala
import server.libs.crypto.{CookieSigner, CookieSignerProvider}
import server.mvc.Cookie.SameSite
import server.{Logger, Mode}
...
object Cookie {
  ...
  object SameSite {
    private[server] val values: Seq[SameSite] = Seq(Strict, Lax)
...
case class DiscardingCookie(name: String, path: String = "/", domain: Option[String] = None, secure: Boolean = false) {
  def toCookie = Cookie(name, "", Some(Cookie.DiscardedMaxAge), path, domain, secure, httpOnly = false)
}
...
trait JWTCookieDataCodec extends CookieDataCodec {
  ...
  private val logger = server.Logger(getClass)
...
object JWTCookieDataCodec {
  ...
  /**
    ...
    */
  private[server] class JWTFormatter(
  ...
  private[server] object JWTIDGenerator {
...
class CookiesModule
  extends SimpleModule((env, conf) => {
    Seq(
      server.inject.bind[CookieSigner].toProvider[CookieSignerProvider],
      server.inject.bind[SessionCookieBaker].to[DefaultSessionCookieBaker],
      server.inject.bind[FlashCookieBaker].to[DefaultFlashCookieBaker]
    )
  })
...
class LegacyCookiesModule
  extends SimpleModule((env, conf) => {
    Seq(
      server.inject.bind[CookieSigner].toProvider[CookieSignerProvider],
      server.inject.bind[SessionCookieBaker].to[LegacySessionCookieBaker],
      server.inject.bind[FlashCookieBaker].to[LegacyFlashCookieBaker]
    )
  })
