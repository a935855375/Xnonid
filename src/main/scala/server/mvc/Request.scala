...
import server.mvc.request.{RemoteConnection, RequestTarget}
...
trait Request[+A] extends RequestHeader {
  ...
  override def hasBody: Boolean = {
    ...
    @inline def isEmptyBody(body: Any): Boolean = body match {
      case rb: server.mvc.Http.RequestBody => isEmptyBody(rb.as(classOf[AnyRef]))
...
/**
  ...
  */
private[server] class RequestImpl[+A](
